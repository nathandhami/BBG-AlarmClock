...
#define DEFAULT_ALARM_SOUND ALARM_SOUND_FOLDER"beep-06.wav"
static string soundFile = DEFAULT_ALARM_SOUND;
...
static LiquidCrystal_I2C lcd("/dev/i2c-1", i2c, en, rw, rs, d4, d5, d6, d7, bl,
		POSITIVE);
...
void Alarm_startProgram()
{
	...
	int thread2 = pthread_create(&display_time_thread, NULL, displayTimeThread,
			(void *) 1);
	if (thread2 != 0) {
		printf("Error creating time thread\n");
	}
...
void checkAlarm(int hour, int minute, int sec, int today)
{
	for (int i = 0; i < size; i++) {
		if (hour == alarm_clock[i].hours && minute == alarm_clock[i].minutes
				&& alarm_clock[i].status && alarm_clock[i].days[today]
				&& sec == 0) {
...
void speechInit(const char* sentence, wavedata_t* file)
{
	...
	int length = sprintf(command, "pico2wave -w wave-files/question.wav \"%s\"",
			sentence);
	command[length] = '\0';
	system(command);
...
void Alarm_addAlarm(Alarm_t alarm)
{
	if ((alarm.hours >= 0 && alarm.hours <= 24)
			&& (alarm.minutes >= 0 && alarm.minutes <= 59)) {
...
{
	for (int i = 0; i < size; i++) {
		printf("%d.  %.02d:%.02d %d, %d, %d, %d\n", i + 1, alarm_clock[i].hours,
				alarm_clock[i].minutes, alarm_clock[i].id,
				alarm_clock[i].difficulty, alarm_clock[i].status, size);
	}
}
...
void* displayTimeThread(void*)
{
	...
	while (true) {
		...
		sprintf(buffer2, "%s %s %d %d", days[weekDay], months[month], monthDay,
				(year + 1900));
		// printf("%s\n", buffer);
		lcd.print(buffer);
		lcd.setCursor(0, 1);
...
void testUser(Alarm_t *alarm)
{
	...
	if (questionType == 0) {
		...
		tempAnswer.erase(remove(tempAnswer.begin(), tempAnswer.end(), '\"'),
				tempAnswer.end());
		...
		if (questionSubType.compare("boolean") == 0) {
			sprintf(answerBuffer, "A. %s, B. %s", answerArray[0].c_str(),
					answerArray[1].c_str());
		} else if (questionSubType.compare("multiple") == 0) {
			sprintf(answerBuffer, "A. %s, B. %s, C. %s, D. %s",
					answerArray[0].c_str(), answerArray[1].c_str(),
...
// Change to 12 hour or 24 hour clock
void toggleHours()
{
	if (hourToggle) {
		hourToggle = 0;
